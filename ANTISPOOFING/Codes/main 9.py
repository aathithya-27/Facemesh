...
# ---------------- CONFIGURATION ---------------- #
ESP32_STREAM_URL = "http://192.168.124.131:81/stream"
ESP32_UNLOCK_URL = "http://192.168.124.131/unlock"
...
def draw_landmark_dots(image, landmarks, color, bbox):
    ...
    for face_landmarks in landmarks:
        for lm in face_landmarks:
            ...
            if x1 <= x <= x2 and y1 <= y <= y2:  # Ensure landmarks are within the bounding box
...
# ----------------- STREAM PROCESSED FRAME TO ESP32 ----------------- #
def send_frame_to_esp32(img):
    """Sends the processed frame back to ESP32 for display."""
    _, img_encoded = cv2.imencode('.jpg', img)
    try:
        requests.post(ESP32_STREAM_URL, files={"image": img_encoded.tobytes()})
...
while True:
    ...
    # Show Live Stream
    cv2.imshow("ESP32 Face Detection & Anti-Spoofing", img)

    # Save and Send Frame to ESP32
    cv2.imwrite(OUTPUT_PATH, img)
    send_frame_to_esp32(img)
